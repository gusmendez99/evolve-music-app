...
const GET_PLAYLISTS_BY_DURATION = "SELECT pl.Name, cast(SUM((t.Milliseconds)/60000.0) as decimal (10,2))   as duration FROM PlaylistTrack plt INNER JOIN Playlist pl ON plt.PlaylistId=pl.PlaylistId INNER JOIN Track t ON plt.TrackId=t.TrackId GROUP BY pl.Name"
...
=======
const GET_COUNT_ARTIST_BY_PLAYLIST = "SELECT ArtistPlayList.Name, COUNT(ArtistPlayList.Name) FROM (SELECT pl.Name as Name, COUNT(t.AlbumId) as ArtistCount FROM PlaylistTrack plt INNER JOIN Playlist pl ON plt.PlaylistId = pl.PlaylistId INNER JOIN Track t ON plt.TrackId = t.TrackId GROUP BY (pl.Name, t.AlbumId)) ArtistPlayList GROUP BY ArtistPlayList.name ORDER BY COUNT(ArtistPlayList.Name) DESC"
const GET_ARTIST_WITH_MORE_GENRES = "SELECT a.Name, COUNT(a.Name) FROM (SELECT a.ArtistId, t.GenreId FROM Artist a INNER JOIN Album al ON a.ArtistId = al.ArtistId INNER JOIN Track t ON al.AlbumId = t.AlbumId GROUP BY(a.ArtistId, t.GenreId)) ArtistGenre INNER JOIN Artist a ON ArtistGenre.ArtistId = a.ArtistId INNER JOIN Genre g ON ArtistGenre.GenreId = g.GenreId GROUP BY (a.Name) ORDER BY COUNT(a.Name) DESC LIMIT 5"

>>>>>>> c7b4b96d73d6d12d50445043d19416477bb2e9b6
